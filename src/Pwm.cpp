...
/******************** Public Method *********************/
/********************************************************/
Pwm::Pwm(const uint8_t pin, const uint8_t channel, const uint16_t frequency) : _pin(pin), _channel(channel), _frequency(frequency)
{
...
void Pwm::handle()
{
  ...
  if (_newValue != _currentValue)
  {
    ...
    if (_fadingSpeed != 0)
    {
      if (abs(currentTick - _oldTick) >= _fadingSpeed)
      {
        if (numStep == NB_STEP)
        {
